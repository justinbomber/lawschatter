...
class EmbedJudgmentPointsUseCase:
    ...
    def _process_jid(self, jid: str) -> bool:
        logger.info(f"處理判決（judgment-level）: {jid}")
        ...
        documents_by_jid = {jid: documents}
        
        before_count = self.vector_store.get_collection_count(self.target_collection)
        self.vector_store.add_judgment_points(documents_by_jid, self.target_collection)
        after_count = self.vector_store.get_collection_count(self.target_collection)
        
        logger.info(f"加入判決 point 到向量資料庫，collection: {self.target_collection}")
        
        self.summary_repo.mark_as_embedded(jid)
        
        logger.info(f"完成處理判決 {jid}，共 {len(documents)} 個摘要類型")
        return True
