...
class Filter(BaseModel):
    ...
    defendants_role: Optional[str] = Field(default=None, description="被告角色定位與功能，說明被告在整體犯罪結構中的位置與職能。【必須使用完整語句描述，字數需達30字以上】。若問句與此項無關直接留空。範例：❌錯誤「車手」；✓正確「依指示前往指定地點提領現金後交付給上游成員之車手角色（俗稱：車手）」")
    A_fact: Optional[str] = Field(default=None, description="法院確認的行為事實與過程，純描述，不含法律用語。敘述法院確認的客觀行為內容與過程，包含行為時間、地點、方式、參與人等要素。【必須使用完整語句描述，字數需達30字以上，不可只填關鍵詞】。若問句與此項無關直接留空。範例：❌錯誤「提供帳戶」；✓正確「被告將其申設之銀行帳戶存摺、提款卡及密碼提供予詐欺集團成員使用，供作收受被害人匯款之用」")
    B_claim: Optional[str] = Field(default=None, description="被告主張與抗辯理由，無論採信與否均完整列出。列出被告於偵查或審理階段的抗辯內容與理由，無論法院是否採信都必須完整保留。若有多次不同說法可取主要版本或最終陳述。不得混入法院評論。【必須使用完整語句描述，字數需達30字以上，不可只填關鍵詞】。若問句與此項無關直接留空。範例：❌錯誤「被告有提供對話紀錄」；✓正確「被告主張其有提供與對方之完整通訊對話紀錄以證明交易過程之真實性與合法性」")
