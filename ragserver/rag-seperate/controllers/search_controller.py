...
class SearchController:
    ...
    def search_documents(self, request: SearchRequest) -> SearchResponse:
        ...
        final_results = self.filter_service.retrieve_results_by_jids(
        ...
        logger.info(f"最終搜尋結果: 總共 {len(final_results)} 個結果")
        if len(final_results) > 0:
            for result in final_results:
                logger.info(f"---> 最終結果: {result.get('payload').get('metadata').get('jid')}")
        else:
